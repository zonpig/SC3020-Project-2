...
def process_whatif_query(user_query, relations, parameters):
    ...
    try:
        ...
        with connection.cursor() as cur:
            ...
            for param in ['seqscan', 'indexscan', 'indexonlyscan', 'bitmapscan', 
            ...
            if parameters.get("random_page_cost"):
                cur.execute(f"SET random_page_cost = {float(parameters['random_page_cost'])};")
            if parameters.get("cpu_tuple_cost"):
                cur.execute(f"SET cpu_tuple_cost = {float(parameters['cpu_tuple_cost'])};")
            if parameters.get("cpu_index_tuple_cost"):
                cur.execute(f"SET cpu_index_tuple_cost = {float(parameters['cpu_index_tuple_cost'])};")
            if parameters.get("cpu_operator_cost"):
                cur.execute(f"SET cpu_operator_cost = {float(parameters['cpu_operator_cost'])};")
            
            # Memory parameters
            if parameters.get("effective_cache_size"):
                cur.execute(f"SET effective_cache_size = '{parameters['effective_cache_size']}';")
            ...
            try:
                ...
                return False, {
                    ...
                    "block_analysis": get_block_analysis(user_query, relations, connection),
