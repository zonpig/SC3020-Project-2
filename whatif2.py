...
import time
import json
from preprocessing import Database,get_plan_summary, get_natural_explanation, get_block_analysis
from psycopg2 import ProgrammingError, OperationalError
from typing import Dict, Tuple, Any

def process_whatif_query(user_query: str, relations: Dict, parameters: Dict) -> Tuple[bool, Dict[str, Any]]:
    """
        ...
        with connection.cursor() as cur:
            ...
            for param, value in parameters.items():
                try:
                    ...
                    elif param in ['work_mem', 'maintenance_work_mem', 'effective_cache_size']:
                    ...
                    print(f"Warning: Failed to set parameter {param} = {value}: {str(e)}")
                    continue
            ...
            try:
                ...
                return False, {
                    ...
                    "query_transformation": {
                        ...
                        "transformation_reason": analyze_transformation(plan_data, user_query),
...
def format_sql(query: str) -> str:
    ...
    """
    keywords = ['SELECT', 'FROM', 'WHERE', 'GROUP BY', 'ORDER BY', 'HAVING', 'JOIN', 'WITH']
    lines = query.split('\n')
    formatted = []
    indent_level = 0
...
def analyze_transformation(plan_data: Dict, original_query: str) -> str:
    ...
    return " and ".join(reasons) if reasons else "Query optimized based on planner settings"
...
if not error:
    ...
    print(f"Transformation Reason: {results['query_transformation']['transformation_reason']}")
